fn main() {
  ...
  let date = std::env::var("RUSTYBOX_BUILD_DATE").ok().filter(|s| !s.is_empty()).unwrap_or_else(|| "unknown".into());
  let args = std::env::var("RUSTYBOX_BUILD_ARGS").ok().unwrap_or_default();
